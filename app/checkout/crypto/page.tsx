  ...
  useEffect(() => {
    ...
    async function createPayment() {
      try {
        ...
        if (active) setPayment(json.payment);
      } catch (e: any) {
        if (active) setError(e?.message || "Failed to create payment.");
      } finally {
  ...
  return (
    <main className="min-h-screen bg-black px-6 py-10 text-white">
      <div className="mx-auto max-w-3xl rounded-3xl border border-neutral-800 bg-neutral-950 p-6">
        ...
        {creating && <p className="mt-4 text-neutral-400">Preparing payment...</p>}
        {error && (
        ...
        {payment && (
          <div className="mt-6 space-y-4">
            <div className="rounded-2xl border border-neutral-800 p-4">
              ...
              <p><strong>USD Value:</strong> ${payment.amountUsd}</p>
            </div>

            <div className="rounded-2xl border border-neutral-800 p-4">
              <p className="mb-2"><strong>Send payment to:</strong></p>
            ...
            <div className="rounded-2xl border border-neutral-800 p-4">
              <p className="mb-2"><strong>Reference:</strong></p>
              <code className="block break-all rounded-xl bg-black p-3 text-sm">
                {payment.reference}
              </code>
            </div>
            ...
            <div className="rounded-2xl border border-amber-800 bg-amber-950/30 p-4 text-amber-200">
              Send the exact amount and keep the transaction signature. Next we’ll add
              automatic wallet-connect verification.
            </div>
          </div>
        )}
      </div>

...
function formatCoveragePercent(value?: number | null, hasCoverage = true) {
  if (!hasCoverage) return "N/A";
...
function RecommendedActionsPanel({
  ...
  hasDeclaredAllocations: boolean;
  sellPressure: SellPressure;
  holderAnalysis: HolderAnalysis;
}) {
  ...
  if (hasDeclaredAllocations && (coverageRatio ?? 0) < 50) {
    recommended.push(
  ...
  if (hasDeclaredAllocations && coverageRatio != null && coverageRatio > 25) {
    positive.push(
  ...
  if (!hasDeclaredAllocations) {
    positive.push("Allocation coverage is not applicable until wallet allocations are declared.");
  }
...
function TrustSealAwardsPanel({
  ...
  const verificationRate = trustData?.verification?.verificationRate ?? 0;
  const allWalletsVerified =
    trustData?.verification?.allWalletsVerified ?? false;
  const tier = trustData?.verification?.tier || "Unverified";

  if (tier === "Platinum" || verificationRate >= 100) {
    awards.push({
      icon: "🏆",
      title: "Platinum Verified",
      description: "All disclosed project wallets are owner verified.",
      tone: "border-amber-400/30 bg-amber-500/10 text-amber-100",
    });
  }

  if (allWalletsVerified && wallets.length > 0) {
    awards.push({
      icon: "🛡",
      title: "Fully Verified",
      description: `${wallets.length} of ${wallets.length} disclosed wallets verified.`,
  ...
  if (typeof score === "number" && score >= 90) {
    if (score === 100) {
      awards.push({
        icon: "💎",
        title: "Perfect Trust Score",
        description: "Project achieved a perfect WEB3MB trust score.",
      ...
      awards.push({
        ...
        description: "Project achieved an A-grade WEB3MB trust score.",
        tone: "border-cyan-400/30 bg-cyan-500/10 text-cyan-100",
      });
    }
  }
  ...
  if (wallets.length > 0) {
    awards.push({
      ...
      description: "WEB3MB is actively monitoring disclosed project wallets.",
      tone: "border-blue-400/30 bg-blue-500/10 text-blue-100",
    });
  }

  if (mismatchWallets === 0 && lowSolWallets === 0) {
    awards.push({
      icon: "⚡",
      title: "Zero Active Wallet Alerts",
      description: "No allocation mismatches or low-SOL warnings detected.",
  ...
  if (!awards.length) {
    awards.push({
      ...
      description: "WEB3MB is collecting verification and wallet health signals.",
      tone: "border-white/10 bg-white/[0.04] text-zinc-200",
    });
  }

  return (
    <div className="mt-8 rounded-3xl border border-amber-400/20 bg-gradient-to-br from-amber-500/[0.08] via-white/[0.035] to-cyan-500/[0.07] p-5 shadow-2xl shadow-amber-950/10 sm:p-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="max-w-4xl">
          ...
          <h2 className="mt-3 text-3xl font-semibold text-white">
            Verified transparency achievements
          </h2>

          <p className="mt-3 max-w-4xl text-sm leading-7 text-zinc-300">
            Awards are issued automatically from live WEB3MB verification,
            wallet health, alert, and trust score signals. These badges help
            investors quickly understand what the project has earned.
          </p>
        </div>

        <div className="rounded-2xl border border-amber-400/20 bg-black/20 px-5 py-4 text-center text-amber-100">
          <div className="text-[10px] font-semibold uppercase tracking-[0.2em]">
            Awards Earned
          </div>
          <div className="mt-2 text-3xl font-semibold">{awards.length}</div>
        </div>
      </div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {awards.map((award) => (
          <div
            key={award.title}
            className={`rounded-3xl border p-5 ${award.tone}`}
          >
...
function ExecutiveRiskSummaryPanel({
  ...
  if (hasDeclaredAllocations && coverageRatio != null && coverageRatio > 0) {
    positiveSignals.push(
      `${formatPercent(coverageRatio)} live coverage ratio detected.`
    );
  }

  if (!hasDeclaredAllocations) {
    positiveSignals.push("Wallet allocations have not yet been disclosed.");
  }
  ...
  const investorAssessment = hasDeclaredAllocations
    ? baseInvestorAssessment
    : `${baseInvestorAssessment} Allocation coverage is not scored yet because wallet token allocations have not been declared.`;
...
export default async function TokenPublicPage({
  ...
  const coverageRatio =
    totalDeclaredTokens > 0 ? (totalLive / totalDeclaredTokens) * 100 : null;

  const hasDeclaredAllocations =
    totalDeclaredTokens > 0 || totalDeclaredPercent > 0;
      ...
      <div className="mx-auto w-full max-w-7xl px-3 py-4 sm:px-4 md:px-6 xl:px-8">
        <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-white/[0.07] to-white/[0.03] p-4 shadow-2xl sm:p-6 md:p-8">
          ...
          <ExecutiveRiskSummaryPanel
            ...
            hasDeclaredAllocations={hasDeclaredAllocations}
          />

            <TrustSealAwardsPanel
            score={score}
            trustData={trustData}
            wallets={wallets}
            mismatchWallets={mismatchWallets}
  ...
  lowSolWallets={lowSolWallets}
  coverageRatio={coverageRatio}
  totalDeclaredPercent={totalDeclaredPercent}
  hasDeclaredAllocations={hasDeclaredAllocations}
/>
          ...
          <div id="overview" className="mt-8 grid min-w-0 scroll-mt-8 gap-4 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <StatCard
              ...
              value={formatCoveragePercent(coverageRatio, hasDeclaredAllocations)}
              hint={
                hasDeclaredAllocations
                  ? "Live disclosed balances versus declared total allocation."
                  : "Coverage is not applicable until wallet allocations are declared."
              }
            />
          </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-black/20 p-5 sm:p-6">
              ...
              <div className="mt-6 grid min-w-0 gap-4 sm:grid-cols-2">
                ...
                <MetricTile
                  ...
                  value={
                    hasDeclaredAllocations
                      ? formatPercent(totalDeclaredPercent)
                      : "Not Declared"
                  }
                />
            ...
            <div className="min-w-0 rounded-3xl border border-white/10 bg-black/20 p-5 sm:p-6">
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                ...
                <MetricTile
                  ...
                  value={
                    hasDeclaredAllocations
                      ? formatNumber(totalDeclaredTokens, 3)
                      : "Not Declared"
                  }
                />
...
function WhyThisScorePanel({
  ...
  const positives = [
    `${verifiedWallets} of ${wallets.length} disclosed wallet${wallets.length === 1 ? "" : "s"} owner verified.`,
    `Verification tier: ${tier}.`,
    mismatchWallets === 0
      ? "No allocation mismatches detected."
      : `${mismatchWallets} allocation mismatch${mismatchWallets === 1 ? "" : "es"} detected.`,
    lowSolWallets === 0
      ? "No low SOL wallet warnings detected."
      : `${lowSolWallets} low SOL wallet warning${lowSolWallets === 1 ? "" : "s"} detected.`,
  ];

  const explanations = [
    `Final Trust Score: ${typeof score === "number" ? score : "—"} / 100.`,
    `Grade: ${grade || "—"}. Status: ${status || "—"}.`,
    `Owner verification coverage: ${formatPercent(verificationRate)}.`,
    hasDeclaredAllocations
      ? `Live coverage ratio: ${formatCoveragePercent(coverageRatio, true)}.`
      : "Live coverage ratio: N/A because wallet token allocations have not been declared.",
    hasDeclaredAllocations
      ? `Declared supply share: ${formatPercent(totalDeclaredPercent)}.`
      : "Declared supply share: Not declared by project.",
  ];

  return (
    <div className="mt-8 rounded-3xl border border-cyan-500/20 bg-cyan-500/[0.06] p-5 shadow-2xl shadow-cyan-950/20 sm:p-6">
      <div className="text-xs uppercase tracking-[0.24em] text-cyan-300">
        Why This Score?
      </div>
      ...
      <p className="mt-3 max-w-4xl text-sm leading-7 text-zinc-300">
        WEB3MB separates owner verification from operational trust. A project
        can be wallet-verified while still receiving a moderate score if live
        balance coverage, disclosure depth, or operational signals need more
        evidence.
      </p>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-5">
        ...
        <MetricTile
          label="Verification"
          value={`${trustData?.verification?.verificationRate ?? "—"}%`}
        />
      </div>
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/10 p-5">
          ...
          <div className="mt-4 grid gap-3">
            {positives.map((item) => (
              <div key={item} className="rounded-2xl border border-emerald-500/20 bg-black/20 p-4 text-sm leading-6 text-emerald-100">
                ✓ {item}
              </div>
            ))}
          </div>
        ...
        <div className="rounded-3xl border border-white/10 bg-black/20 p-5">
          ...
          <div className="mt-4 grid gap-3">
            {explanations.map((item) => (
              <div key={item} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-sm leading-6 text-zinc-300">
                • {item}
              </div>
            ))}
          </div>

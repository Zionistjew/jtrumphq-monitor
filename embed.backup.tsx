...
async function getLeaderboardRank(slug: string) {
  try {
    const baseUrl = await getBaseUrl();
...
export default async function EmbedWidget({
  ...
  const [trust, leaderboardRank] = await Promise.all([
    getTrustData(slug),
    getLeaderboardRank(slug),
  ]);
  ...
  const awards = buildAwards({
    ...
    verifiedWallets: Number(verifiedWallets || 0),
  });
    ...
    <div className="flex min-h-screen items-center justify-center bg-transparent p-3">
      ...
      >
        <div className="p-6">
          ...
          <div className="mt-5 rounded-2xl border border-yellow-300/20 bg-yellow-500/10 p-5">
            <div className="text-[11px] uppercase tracking-[0.25em] text-yellow-200">
              Trust Seal Awards
            </div>
            ...
            {awards.length ? (
              <div className="mt-4 flex flex-wrap gap-2">
                {awards.map((award) => (
                  <div
                    key={award.label}
                    className={`rounded-full border px-3 py-2 text-xs font-black ${award.tone}`}
                  >
              ...
              <div className="mt-3 rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-zinc-300">
                Transparency monitoring active. Awards unlock as verification
                and trust score milestones improve.
              </div>
            )}
          </div>
